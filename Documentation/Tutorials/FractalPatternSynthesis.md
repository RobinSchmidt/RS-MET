...
Iteratively applying these rules to the initial seed "A", the string would develop as follows:
...
First, we want to look at how we can make sure that the starting point of the curve coincides with the endpoint. One simple way to ensure that is to use a seed that itself describes a closed curve. Assuming that the turn angle is set to 90�, a command string like `F+F+F+F` would draw a square. The endpoint of the turtle would coincide with the start point, so we have a closed curve. There's a little catch here, when we use this for sound synthesis. In the creation of graphical drawings, we usually trace out the curve just once - so in this setting, the command string above would be good enough to draw a simple square. But in sound synthesis, we'll trace out the curve again and again. Note that, after tracing out the curve, the turtle will be looking into the downward direction instead of rightward, as it initially did. So if we let the turtle run a second time through the command string without resetting its state before, we'll draw a second square below the first. In the third run, we'll draw a third square left to the second and in the fourth run we'll draw a fourth square on top of the third. Only after these four runs, the turtle will be again in its initial position *and* look into its initial direction. This can be useful, but you should be aware of it. If you want to trace out the same initial square again and again, one way to fix it would be to reset the turtle's state to the initial state after each run through the curve. Another way that doesn't require resetting would be to append another `+` to our command string, so it would become `F+F+F+F+`. This added `+` fixes the direction of the turtle after running through the command string. You need to watch out for this when using L-system definitions that you grab somewhere from the web or a book. There, a graphical setting is assumed in which the curve is traced out just once anyway, so there's no consideration of this effect and final "direction fixes" are usually absent.
...
ToDo:  
-allow user to define a set of named rule-sets, for example A: F = F+F; B: F = F-F and instead of specifying a number of iterations, the user specifies an iteration/application string like AABBBA which means: apply A two times, then B three times then A once. support also a syntax like A2B3A. maybe these "applications strings" could also be generated by L-systems? that would be sooo meta! :-) ...but maybe not that useful, because application strings should not be as monstrous as the actual turtle command-strings - 10 rule applications are actually already a lot (although, that depends on the complexity of the rules). maybe have a "Radomize" button that produces random application strings (or maybe forward/backward buttons to step through a bunch of randomized application strings)
