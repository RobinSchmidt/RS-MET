...
bool rotes::testUnivariateScalarFunction()
{
  ...
  */
  ...
  // (-52 - 23*x + 21*x^2 -7*x^3 + 1*x^4).factor()
  // (x^2 - 4*x + 13)*(x + 1)*(x - 4)
  ...
  double xr;
  xr = p.findRootViaNewtonNonRobust(0.0);    ok &= xr == -1;  // 8 iterations
  xr = p.findRootViaChebychevNonRobust(0.0); ok &= xr == -1;  // 7 iterations
  ...
  xr = p2.findRootViaRidders(-3.0, +6.0); ok &= xr == -1;   // 10 iterations (there 3 roots  between -3...+6)
  ...
  // todo: merge with similar tests for rapt - requires to implement the Ridders algo in rapt,
  // too. The numerical analysis algos should all be templatized and go to rapt anyway
