#ifndef RS_SORTANDSEARCH_INL
#define RS_SORTANDSEARCH_INL
...
namespace RSLib
{
  ...
  int rsFindFirstOccurrenceOf(const T *buffer, const int length,
                              const T &elementToFind, const int searchStart = 0)
  {
                              ...
                              const int patternLength, const int searchStart)
  {
    ...
    {
      if( rsAreBuffersEqual(&buffer[shift], patternToMatch, patternLength) )
        return shift;
    }
    return -1;
  }
