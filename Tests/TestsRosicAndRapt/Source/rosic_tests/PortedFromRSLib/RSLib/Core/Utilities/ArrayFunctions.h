#ifndef RS_ARRAYFUNCTIONS_H
#define RS_ARRAYFUNCTIONS_H
...
namespace RSLib
{
  ...
  // todo: rename file to ArrayFunctions, maybe rename some functionsaccordingly
  ...
  */
  ...
  RS_INLINE void rsSwapDataBuffers(void *buffer1, void *buffer2, void *bufferTmp, int sizeInBytes);
  ...
  template <class T>
  T rsWeightedSum(T *w, T *x, rsUint32 length);

  /** Forms a weighted sum of the two buffers. */
  template <class T>
  RS_INLINE void rsWeightedSum(T *buffer1, T *buffer2, T *result, int length, T weight1, 
    T weight2);
