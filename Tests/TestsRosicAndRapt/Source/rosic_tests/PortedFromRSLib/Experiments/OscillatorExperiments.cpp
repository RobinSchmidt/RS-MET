...
void phaseShapingSkew()
{
  ...
  vector<double> p = createPhase(N, f, fs); // Raw phase (as input for computing shaped phase)
  vector<double> ps(N);                     // Shaped phase (for optional plotting)
  vector<double> y(N);                      // Output signal (for wav-writing)
  ...
  // Apply the the phase-shaping and generate the signal:
  //int n;
  ...
  for(int n = 0; n < N; n++)
  {
    ...
    y[n]  = amp * sin(2*PI*ps[n]);
    //ps[n] = phaseShapePower(p[n], pow(2.0, 2*a[n]));  // new, test
  }
  rsPlotVector(ps);
  rsPlotVector(y);
  writeToMonoWaveFile("PhaseShapeRatSine.wav", &y[0], N, (int) fs, 16);
  ...
  */
  ...
  // Observations:
  // -The signal "PhaseShapeRatSine.wav" morphs from a highpassed downward saw through sine to a
