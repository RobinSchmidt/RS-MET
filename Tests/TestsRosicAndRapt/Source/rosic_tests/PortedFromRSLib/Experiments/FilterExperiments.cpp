...
void allpassFDN()
{
  // Under construction. I try to implement the completion algorithm in secion C in the paper:
  ...
  // for the simplemost case of 1 input, 1 output and 2 delaylines with a 2x2 feedback matrix. The 
  // goal is to find suitable input and output vectors B,C and a suitable direct gain D such that 
  // the whole system becomes allpass. It's a first step toward more complicated cases.
  ...
  Real g      = 0.8;                             // Feedback loss coeff
  ...
  Real g1 = pow(g, delays[0] / delays[0]);  // == g, but just to emphasize the pattern
  Real g2 = pow(g, delays[1] / delays[0]);  // longer delaylines need smaller feedback coeffs
  Mat  G  = Mat(2,2,  {g1,0, 0,g2} );       // G = diag(g1, g2)
  Mat  A  = H*G;                            // ToDo: Try also A = G*H. What's the difference?
  // In (1), it says that a unilossless matrix is multiplied from the right by a diagonal matrix G:
  // A = U*G. That actually amounts to scale the columns of A by the respective diagonal elements 
  // of G? Dos that make sense? I think, uniform decay would be achieved by scaling the elements of
  ...
  // implementation or maybe it doesn't matter? I think, scaling the columns amounts to scaling the 
  ...
  // The singular values are actually precisely our g1, g2 values. Interesting observation. We
...
void biquadDesignVicanek()
{
  ...
  FDF::mvBandpassSimple(wc, 3.0, false, &b0, &b1, &b2, &a1, &a2); plotFreqResp();
  FDF::mvBandpassSimple(wc, 3.0, true,  &b0, &b1, &b2, &a1, &a2); plotFreqResp();
...
void stateVariableFilters()
{
  ...
  stateVariableFilterMorph();  // Trying to figure out how to morph between types
