...
void biDirectionalStateInit()
{
  ...
  // Note that when using the xplicit formula to compute the tail s(n), it works well only when b 
  ...
  double yOld  = yf[N-1];  // last output of filter, equals its y-state
  double k = 1/b; 
  ...
  /*
  double y0  = yf[N-1];  // last output of filter, equals its y-state
  //yL = flt.getSample(0); // test
  ...
  */
  ...
  // todo: compute and plot the tail generated by the closed form expression
  double ta[Nt];    // analytic tail
  //double p = a*yOld / (b*b - 1);
  //double c = -p*k;  // == -p/b
  for(n = 0; n < Nt; n++)
    ta[n] = c*pow((1/b), n-1)  -  (a * yOld * (pow(b,n) - pow((1/b),n))) / (b*b - 1);
  // ok - this looks good (if b is negative power of 2 - otherwise numercial error messes up the
  // analytic solution) - it is equal to t but one sample shifted - when plugging n=1 into the 
  // formula, we can indeed compute our desired state variable s1:

  //s1 = c - p*(b-k); 
  // check, if this formula is equivalent to the ad-hoc formula s1 = a*y0*(b-k) / ((b*b-1)*(k*k-1)) 
  // used above - if so, we know why it works - check also, if it can be simplified further
  //err = s1 - t[0]; // should be zero (up to roundoff)
