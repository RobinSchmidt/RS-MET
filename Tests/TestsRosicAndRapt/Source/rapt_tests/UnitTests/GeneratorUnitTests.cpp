...
bool samplerAmpEnvTest()
{
  ...
  // chain or append another Amplifier (which is the case here):
  setupCommonSettings();
  ok &= numAmps(se) == 0;
  ...
  ok &= numAmps(se) == 1;
  se.setRegionSetting(0, 0, OC::amplitudeN, 200.f,  2);  // appends 2nd amp with 200% gain
  ok &= numAmps(se) == 2;
...
bool samplerMidiModulationsTest()
{
  ...
  //  Should we?
  // -Set up a patch with a highpass and a lowpass and route cc74 to both cutoffs. ...actually, 
  //  let's do that with an sfz preset
