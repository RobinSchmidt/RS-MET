double rsAnd(double pa, double pb, double cor)
{
  ...
  return pa*pb + cor*d;
  // this is the formula implemented in rsCorrelation but solved for P(A,B)
}
...
{
  return (pAB-pA*pB) / sqrt(pA*(1-pA)*pB*(1-pB));
...
-maybe we can make a sort of graph with nodes representing events (with their probabilities) and
 edges representing correlations (is this what bayesian belief networks do? look up...)
