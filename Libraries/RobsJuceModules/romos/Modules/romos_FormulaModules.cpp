...
- Maybe make a formula module with only a single inputs and let the user decide on the GUI if they
  want direct evaluation or a table based evaluation. For the table, let the user set the min, max,
  numSamples/resolution, interpolation method, etc.

- Make a formula module that has multiple outputs whose pin names can be assigned by the user on 
  the GUI just like the input pins.

- Maybe make a formula with single input and multiple outputs where the user can for each output
  individually select the evaluation method (direct or table). For a formula with multiple inputs,
  we would have to use nD tables which is complicated, so maybe for the time being, allow only 
  direct evaluation for multi-input functions.
