...
retrieving values form it at arbitrary positions by means of interpolation. The user can select
between a couple of built in standard wvaeforms or pass an arbitrary waveform. */
...
public:
  ...
  /** Not yet implemented...apparently...and it seems to make more sense to implementit in the 
  realtime code for the phase-increment anyway because this is something we want to modulate.
...
INLINE double MipMappedWaveTable::getValueLinear(int integerPart, double fractionalPart, 
  int tableIndex)
{
  // ensure, that the table index is in the valid range:
  if(     tableIndex <= 0)      tableIndex = 0;
  else if(tableIndex>numTables) tableIndex = 11;  // use numTables-1
  // maybe use rsClip(tableIndex, 0, numTables-1)
...
// todo: clean up the commented old code
