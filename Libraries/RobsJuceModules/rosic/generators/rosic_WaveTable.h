...
/** A class for generating and storing a single-cycle-waveform in a lookup-table and retrieving 
values form it at arbitrary positions by means of interpolation. It doesn't do any anti-aliasing, 
so it's recommended only for use in low-frequency oscillators (LFOs). For mip-mapped wvaetables 
that can be used in audio oscillators, you may want to look at MipMappedWaveTableStereo.

\todo: factor out the stiff that we have in common with MipMappedWaveTableStereo, maybe into a
WaveTableManipulator class */
