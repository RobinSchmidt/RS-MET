...
public:
  ...
  /** Inverts accent settings (using logical not with current settings). */
  void invertAccents() { patterns[activePattern].invertAccents(); }

  /** Inverts slide settings (using logical not with current settings). */
  void invertSlides()  { patterns[activePattern].invertSlides(); }

  /** Inverts octave settings (negates their signs). */
  void invertOctaves() { patterns[activePattern].invertOctaves(); }

  /** Swaps accents with slides. */
  void swapAccentsWithSlides() { patterns[activePattern].swapAccentsWithSlides(); }

  /** Computes exclusive or (XOR) of accents and slides and assigns accents to the results. */
  void xorAccentsWithSlides()  { patterns[activePattern].xorAccentsWithSlides(); }

  /** Computes exclusive or (XOR) of accents and slides and assigns slides to the results. */
