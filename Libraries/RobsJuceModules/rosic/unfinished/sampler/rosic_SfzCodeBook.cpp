  ...
  int* startPos, int* endPos)
{
  ...
  findOpcodeValueString(code, Opcode::set_ccN, idx, (int) searchStart, (int) searchEnd, 
    startPos, endPos);
  // We know that set_ccN opcodes must always occur in the <control> section so we don't need to
  // search through the whole text. Scanning only the control section may make a difference between 
  // searching through a string with a few hundreds of chars vs one with a few thousands or even 
  // tens of thousands. The control section is often only a small part of the whole sfz.
  // Speaking of optimizations, it may also be a good idea to operate on const char* arrays rather
  // than std::string in order to avoid conversions from juce::String to std::string within
  // ToolChain. 
}
