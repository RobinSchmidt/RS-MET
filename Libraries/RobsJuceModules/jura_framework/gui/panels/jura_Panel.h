...
underlying coordinate-system (albeit possibly implicit and invisble). It provides means to set up 
maximum values in the x- and y-direction, to select a currently shown range within this maximum 
range and has functions for conversion between coordinates in the panel's own intrinsic coordinate 
system and component-coordinates. */
...
public:
  ...
  /** Constructor. */
  Panel(const juce::String &componentName = juce::String("Panel"));
  ...
  virtual void toPixelCoordinates(double &x, double &y) const;

  /** Function for converting the x- and y-coordinate values into the corresponding coordinates
  in the component (single precision version).*/
  virtual void toPixelCoordinates(float &x, float &y) const;
