...
void rsWaveEquation1D_Proto<T>::stepLeapFrog(std::vector<T>& u, std::vector<T>& u1)
{
  ...
  // Experimental code to handle more general boundary conditions (firstly, on the left end):
  //T c = T(0.5);                      // A coeff that when 0, fixes the end and when 1, lets it follow u[1]
  ////uNew[0] = c*u[1] - (1-c)*u1[0];  // Was generated by GitHub Copilot. i don't understand it.
  //uNew[0] = u[0] + c * (u[1]-u[0]);  // My idea as explained in the comments.
  // When we leave out this code (or comment it out), the boundary values will not be updated at 
  // all so they will forever remain at their initial values. This corresponds to a fixed boundary.
  // If the initial value is zero, it corresponds to a zero displacement boundary condition. For 
  // other values, I think it means that the string is just slanted (when the values are unequal 
  // at both ends). ToDo: Try it! Maybe try u[0] = 1, u[M-1] = -1.
