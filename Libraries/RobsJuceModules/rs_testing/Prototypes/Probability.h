...
class TriLogic
{
  ...
  F  F F F
  U  U F U   */
  static char _and(char a, char b);
  ...
  F  T F U
  U  T U U   */
  static char _or(char a, char b);

  // we need the underscores because otherwise the gcc compile (on linux) balks
