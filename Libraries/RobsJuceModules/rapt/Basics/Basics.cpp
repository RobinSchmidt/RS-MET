...
- Maybe make a subfolder "DevTools" into which we then move DebugTools and the Plotting stuff. 
  ...
  Logging.h, ErrorHandling.h, Sanitization.h. There, we could also include the heap allocation 
  logging stuff which is currently in the research repo in \Projects\CppExperiments. It could also
  contain stuff for instrumentation, benchmarking, etc. (although the latter may better be placed 
  outside the library). We could also have a logging allocator there.
