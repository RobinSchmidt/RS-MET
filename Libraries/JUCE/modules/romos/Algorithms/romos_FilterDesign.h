...
namespace romos
{
  ...
  // The following functions are for calculating biquad coefficients for filters realizing the difference equation:
  // y[n] = b0*x[n] + b1*x[n-1] + b2*x[n-2] - a1*y[n-1] - a2*y[n-2]
  // the coeffs array is assumed to be of the form: 
  // coeffs[0] = b0, coeffs[1] = b1, coeffs[2] = b2, coeffs[3] = a1, coeffs[4] = a2

  /** Assigns the biquad coefficients to produce a filter that passes the signal through unchanged. */
  ...
  /** Second order allpass filter (RBJ cookbook design). q determines slope of the phase response at the -180 degree frequency. */
  ...
  where the "coeffs"-array is organized as follows: coeffs[0]=a1, [1]=k, [2]=c0, [3]=c1, [4]=c2, [5]=c3, [6]=c4
  "f" is the cutoff frequency, mode is the filter-mode, "r" is the amount of resonance (1.0 means self-oscillation) and "ag" is the amount
  of automatic gain compensation that counteracts the gain loss at high resonances (1.0 means full compensation)
  */
