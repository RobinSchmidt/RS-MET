...
namespace romos
{
  ...
  { 
    PeriodicLinearRamp *ramp = static_cast<PeriodicLinearRamp*> (module);

    // generate output signal:
    *out = ramp->phases[voiceIndex];

    // phase increment and wraparound:
    ramp->phases[voiceIndex] += *in1 * processingStatus.getSystemSamplePeriod();
    while( ramp->phases[voiceIndex] >= 1.0 )
      ramp->phases[voiceIndex] -= 1.0;
    while( ramp->phases[voiceIndex] <  0.0 )
      ramp->phases[voiceIndex] += 1.0;
  }
  void PeriodicLinearRamp::resetVoiceState(int voiceIndex)
  {
    ModuleAtomic::resetVoiceState(voiceIndex);
    phases[voiceIndex] = 0.0;  // introduce a startphase later (as GUI parameter)
  }
  void PeriodicLinearRamp::allocateMemory()
  {
    ModuleAtomic::allocateMemory();
    phases = new double[getNumVoices()]; 
  }
  void PeriodicLinearRamp::freeMemory()
  {
    ModuleAtomic::freeMemory();
    delete[] phases;
    phases = NULL;
  }
  CREATE_AND_ASSIGN_PROCESSING_FUNCTIONS_1(PeriodicLinearRamp); 
