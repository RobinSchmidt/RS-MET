...
namespace romos
{
  ...
  /** We use identity modules to realize input and output - but we make some special subclasses in order to override the default 
  module names. maybe factor out a baseclass PinModule */
  ...
  /** Outputs the current system sampling period, that is: the distance between two samples in seconds. This is equal to the reciprocal of 
  the samplerate. */
  ...
  /** Kills the voice when the maximum absolute value of the input signal remains below a threshold for some specified time. */
  ...
  /** Outputs a number that represents some user parameter. It facilitates presentation to the user by defining a range and mapping. */
  ...
  public:
    ...
    virtual void resetVoiceState(int voiceIndex); // later: set current-value to target value there (smoothing)
    ...
    refuse to take values when setting one at a time that violates the constraint. For example, when the currrent maximum is 1 and you 
    try to set a new minimum > 1, it will actually set the new minimum to 1, etc. */
    ...
    */
    ...
    /** Given the 4 snapshot-indices s1...s4 at 4 corners of a square and an x- and y-coordinate (between 0...1), this function sets up
    the value of the parameter to an interpolated value between the 4 corner snapshot values. The interpolation is bilinear in the 
    normalized parameter domain. */
    virtual void setValueFromSnapshots(int topLeftIndex, int topRightIndex, int bottomLeftIndex, int bottomRightIndex,      
      double x, double y);
  ...
  protected:
    ...
    /** Enforces the values of minValue, maxValue, etc. to satisfy certain constrains (for exmple minValue <= maxValue, etc.). */
    virtual void enforceConsistencyOfValues();

    /** Maps a normalized value in the range 0...1 (both inclusive) to the actual parameter range minValue...maxValue (both inclusive) 
    according to the selected mappingFunction. */
    double mapNormalizedValue(double normalizedValue);

    /** Given an actual parameter value in the range minValue...maxValue (both inclusive), this function returns the corresponding 
    normalized value in the range 0...1 (both inclusive). */
    ...
    // maybe have a normalizedValue member - it would make snapshot-interpolation morde efficient (because this is done in the normalized 
    // domain), but it also introduce redundancy in the data ...we'll see
