...
namespace romos
{
  ...
  This class is used to map numeric module identifiers for module types to and from indetifiers represented as strings. The former are 
  mainly used in the DSP code where fast access matters whereas the latter are mainly used for persistence purposes, where we want to have
  human readability. It is implemented as a singleton class.
  ...
  */
  ...
  public:
    ...
    /** Defines a list of numeric identifiers to be used to quickly identify the type of a module inside the DSP code. */
    ...
    /** Returns true when the module type given by typeIdentifier has an editable name, false otherwise. Some basic modules do not allow
    their name to be edited such as adders, multipliers, unit delays, etc. */
    static bool isModuleNameEditable(int typeIdentifier);


    // todo: make a similar function doesModuleBlockHaveHeader() - get rid of the flag in Module class


    /** Returns true, if the modules with given typeCode have a GUI editor - some very simple modules such as adders, et.c don't. */
    static bool hasModuleTypeEditor(int typeCode);

    //-------------------------------------------------------------------------------------------------------------------------------------
    // access to the sole instance:

    /** Returns the sole instance of the ModuleTypeRegistry class (it's a singleton class). It will also create and initialize the 
    instance before, if necessary (i.e. if its hasn't been done already). */
    static ModuleTypeRegistry* getSoleInstance();

    /** Deletes the sole instance of the ModuleTypeRegistry class, if existent. We can use this to manually clean up heap memory 
    which would otherwise be cleaned up automatically on shutdown. Manual cleanup is preferable in order to not distract the memory leak 
    detection of MSVC during development/debugging. */
  ...
  Min, Max, Floor, Ceil, Round, Abs, Angle(x, y), Length(x, y), ToRA(x, y) (radius/angle), ToXY(r, a), Equal, InEqual, Greater,
  GreaterOrEqual, Less, LessOrEqual, And, Or, Not, Nand, Nor, Xor
  ...
  detection:
  -zero-crossing finder (outputs a trigger signal and an offset to indicate subsample location of the crossing)
  -maximum-finder: x[n-1] > x[n] && x[n-1] > x[n-2] - output a trigger and an offset (found by parabolic interpolation)
   -min-finder likewise
  ...
  UpSample, DownSample
  -when connecting modules with different oversampling factors, automatic conversion is done by zero insertion or discarding
