...
namespace romos
{
  ...
  This class is used to represent the special top-level, or root module. This usually represents a complete instrument or effect unit. It 
  is a CompositeModule with a bit of additional functionality that applies only to and instrument/effect as a whole. 
  
  
  It also stores some 
  global and per-voice information such as system sample-rate, the tempo in BPM, the notes that are currently active (for each voice), etc. 
  Some modules access these informations.  ...nah not true - we have a singleton for this
  ...
  */
  ...
  public:
    ...
    /** Disconnects the input pins of our output modules (if connected) - needed for clean-up when recalling a patch. */
    ...
    /** Overriden to avoid deletion of I/O modules and additionaly delete connections that go directly from inputs to outputs. */
    virtual void deleteChildModule(Module *moduleToDelete, bool updateHasDelayedConnectionFlag = true);

    /** Overriden to avoid attempts to re-connect our outside connections (which - by definition - don't exist). */
  ...
  protected:
    ...
    /** Does initialization stuff for input buffers and input-modules (memory allocation, setting up the pointers, etc.). To be called from
    the constructor. */
    ...
    // \todo maybe use a special processing function that uses a flat list/array of all modules that represents the whole instrument as
    // one monolith - that is, without any structuring via containers. this array can contain just pointers to the modules that are inside
    // the containers - no actual duplication is necessary. doing it like this avoids the container-overhead
    // we should make sure that this array is always properly sorted - but that's straightforward -> just traverse the tree and include
    // the I/O modules -  when moving a moduel inside a container, the container can request its toplevel module, access the flat array 
    // there and replace a subarray with the re-ordered subarray
    // we can assign the source/target module pointers in the connections for container modules to the container's respective I/O modules
    // ->as these are first class identity modules, it should work for the DSP side, for the GUI side, we must take care that the 
    // connection still returns the container-module when the GUI requests the pointer (maybe have another function 
    // getApparentSourceModule or something)
  ...
  template <class SampleType>
  INLINE void TopLevelModule::getBlockOfSampleFramesStereo(SampleType *inOutL, SampleType *inOutR, int numFrames)
  {
    if( numFrames < 1 )
      return;
    ...
    // when this block-processing function is called immediately after receiving a note-event, we must process the 1st sample-frame 
    // separately using the per-sample processing function to make sure that the voiceAllocator's note-on/off flags are reset to false
    // after the 1st sample:
