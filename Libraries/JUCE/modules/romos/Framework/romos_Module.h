...
namespace romos
{
  ...
  /** In its current state, it's only a data class with rudimentary behaviour - maybe turn this into a proper class later */
  ...
  This is the baseclass for all modules. A module is seen from the outside as a black box that has a number of inputs and a number of 
  outputs for both, audio signals and events. The module baseclass defines an interface for all concrete modules to adhere to and 
  provides methods for keeping track of the wiring between modules. 

  \todo: 
  -look into pd for inspiration
  -Module* getClone() operation (virtual)
  -array of slave-modules for polyphonic operation
  -use unsigned int consistently
  ...
  */
  ...
  public:
    ...
    sorted ofter the move. If there is no parent, this has no effect, but if there is a parent, this ensures that after the move, the 
    parent has the child-array sorted in the desired evalution order as determined from the coordinates of the children. */
    ...
    /** Disconnects all our input pins that receive their input from the given sourceMdouleToDisconnect. */
    virtual void disconnectInputPinsWithInputFrom(romos::Module *sourceModuleToDisconnect);

    /** Disconnects all our input pins that receive their input from the given sourceMdouleToDisconnect and given outputPinIndex. */
    virtual void disconnectInputPinsWithInputFrom(romos::Module *sourceModuleToDisconnect, int outputPinIndex);
    ...
    /** Updates our member variables double **inputs and unsigned int *inFrameStrides according to the incoming connections such that
    inputs[i] is a pointer to the i-th output (for voice 0  and frame 0) of the connected source-module and inFrameStrides[i] is the 
    distance between successive output frames in the source module (this is equal to the source-modules' number of output pins. */
    ...
    /** Returns voiceAllocator.getNumVoices() when "this" module is polyphonic modules, and 1 when it is monophonic. */
    ...
    /** Returns the top-level module in the whole module hierarchy. It will resolve "this" for modules that don't have a parent and to a 
    recursive call to getTopLevelModule on the parent for modules that do have a parent. */
    virtual romos::Module* getTopLevelModule();

    /** If "this" module is a child module, of some parent module, the function returns the index of "this" module with the parent's
    array of children (which determines evaluation order), -1 is returned, when this module has no parent. */
    ...
    /** This is an inlined version of getNumInputs and it is for internal use only - it will return wrong results in case of I/O modules. 
    The internal code knows this and handles it appropriately - client code should only use the virtual getNumInputs function instead. We 
    have this inlined version to make it fast to call from DSP code. But inlining also implies that it can't be virtual which is why we 
    can't directly inline getNumInputs. The reason we need to override getNumInputs/Outputs in the I/O modules is because for I/O modules, 
    the numInputs member does actually represent the number of inputs of the parent container-module. This is certainly a major quirk, but 
    it is required to make the DSP code fast.  ...maybe someday we find a cleaner solution to this problem... */
    ...
    /** Returns the module that should be used as actual source for the output-signals. Normal modules will return themselves, but some
    modules act as proxy and return the module that is connected to their inputs instead. When this is called for a container, for 
    example, the container will return itself but instead invoke getConnectableSource */
    //virtual Module* getConnectableSourceModule(Module *attemptedSourceModule);

    /** Similar to getConnectableSourceModule but returns the index that is actually be used as source output index, when this 
    is called */
    //virtual int getConnectableSourceOutputPinIndex(int attemtedOutputPinIndex);

    /** Maps a pointer to an apparent source-module/pin-index for a connection to the actual source-module/pin-index that is used in 
    the processing code, resolving all proxy-modules that may be in between. It should be called like 
    sourceModule->mapApparentSourceToProcessingSource(sourceModule, sourceOutputPinIndex)
    such that the member-function of "sourceModule" is called with a reference to the "sourceModule"-pointer itself. Normal (non-proxy) 
    modules will do nothing and leave the "sourceModule" pointer-reference at "this" and also leave the pin-index as is. But a container, 
    for example, when acting as source for a connection, will set the sourceModule-pointer reference to the output module that corresponds 
    to the attempted "sourceOutputPinIndex" and set the "sourceOutputPinIndex" to 0 (output modules have only one one (invisible) output 
    pin). But that's not yet the end of the chain: after setting the modulePointer/pinIndex, the container calls
    sourceModule->mapApparentSourceToProcessingSource(sourceModule, attemtedOutputPinIndex) again - and because the module-pointer points 
    now to an AudioOutputModule, the overriden implementation there will be called. This in turn will again redirect the pointer, this time 
    to the modulePointer/pinIndex that acts as the source for the output module. This chain of modulePointer/pinIndex manipulations will 
    end when it arrives at an elementary non-proxy module - this is then the actual modulePointer/pinIndex pair that will be used by the 
    signal processing code. */
    virtual void mapApparentSourceToProcessingSource(Module * &sourceModule, int &sourceOutputPinIndex) { }

    /** Given a modulePointer/pinIndex pair, this function will possibly modify the pair such that it represents the module/index pair as 
    apparent on the GUI. This realizes the inverse mapping of mapApparentSourceToProcessingSource - see explanation there for more 
    details. */
    ...
    /** Returns a pointer to the double value that represents the value of the given output-pin in voice 0 and frame 0. */
    virtual double* getOutputPointer(int pinIndex) const; // to be overriden by PointerRedirectModule

    /** Returns the distance between succsessive frames in the output signal buffers. Normally, this is the same as the number of 
    output-pins, but PointerRedirectModules (and subclasses thereof) require some special treatment which is why the function is overriden 
    there. A PointerRedirectModule returns the frame-stride for its connected source-module, by delegating the call to it. */
    virtual int getOutputFrameStride() const; // to be overriden by PointerRedirectModule

    /** Returns the distance between succsesive voices in the output signal buffers. Normally, this is the same as the number of 
    output-pins times the output buffersize, but PointerRedirectModules (and subclasses thereof) require some special treatment
    @see getOutputFrameStride */
    ...
    */
    ...
    /** Returns true if the input pin with given index is connected to some output pin, flase otherwise. */
    ...
    /** Returns true when this module has an incoming connection from the given output pin of the given source module. */
    bool hasIncomingConnectionFrom(const romos::Module *sourceModule, int sourceOutputPinIndex) const;

    /** Return true when this module has one or more delayed incoming connections. */
    bool hasDelayedIncomingConnection() const;

    /** Returns a vector with all modules that are connected to this module where this module is the source of the connections and the 
    elements of the vector are the targets of the connections. */
    std::vector<romos::Module*> getConnectedTargetModules() const;

    /** Similar to getConnectedTargetModules() but includes only those modules that are connected to the given output pin. */
    ...
    /** Returns an array of pointers to all outgoing audio connections. Because modules do not keep track of their outgoing connections, 
    the function must iterate to "this" module's parent's children and for each child, check it's incoming connections if they have 
    "this" module as source-module. */
    virtual std::vector<AudioConnection> getOutgoingAudioConnections();

    /** Returns an array of pointers to all outgoing audio connections that emanate from the passed output pin. */
    ...
    /** Given the passed array of modules, it assigns the reference variables to the midpoint of the rectangle that is determined by the
    extreme coordinates (as given by getExtremeCoordinates()). */
    ...
    /** Function that is called whenever the module should resets the internal state of the module (for example filter history, 
    oscillator phase, etc) for a particular voice. */
  ...
  protected:
    ...
    /** Assigns the two processing function-pointers (for frame-based and block-based processing). Subclasses need to override this and in
    their overriden function check for the "polyphonic" flag in order to assign the monophonic or polyphonic version of each of the 
    functions. */
    ...
    /** To be overriden by concrete subclasses to initialize the names of the pins, allocate ressources, etc.. */
    virtual void initialize() = 0;

    /** May be overriden by concrete subclasses to clean up allocated ressources. The baseclass method just calls freeMemory(). */
    ...
    bool  hasHeaderFlag;      
      ...
      // function instead that switches on the typeCode or similar
  ...
  /** Decides for two modules (given by their pointers), if the first is considered to become before the second or not. This is needed
  to sort module-pointers according to their desired evaluation order which shall be governed by the x- and y coordinates of the 
  respective modules. Modules that are to the left of other modules (that is, with lower x-coordinate) are evaluated before those other 
  modules. Inside a column with equal x-values, the upper modules (those with lower y-values) are evaluated before others. When two modules
  have equal x- and y coordinates, the evaluation order is undefined - this should be prevented by presenting the user a warning in such 
  cases. */
  bool modulePointerLessByXY(const romos::Module *left, const romos::Module *right);

  /** Returns true when the the passed vector of modules contains one (or more) modules of the given type (the code is interpreted 
  according to the ModuleTypeRegistry::moduleTypeCodes enumerator. */
  bool containsModuleOfType(const std::vector<romos::Module*> &modules, int typeCode);


  /** Similar to modulePointerLessByXY but considers y-coordinate as more important than x-coordinate. */
  //bool modulePointerLessByYX(romos::Module *modulePointer1, romos::Module *modulePointer2);

  /** A helper function for degubbing purposes - writes the state (name, numChildren, parent, numIns/Outs, values of ins/outs, etc.) of the 
  module to the standard output. The module should be passed as void pointer so we can call it from the process functions. The second 
  parameter specifies whether the program should pause and wait for the user to hit a key until it continues. */
  ...
  /** A function that can be called when things occur that shouldn't. It will open a message box with the error text and give the user the 
  option to write the message into a logfile. ....later. */
