...
{
  rosic::fillWithValue(inputs[0][0], numFramesToProcess, freq);
  rosic::fillWithValue(inputs[0][1], numFramesToProcess, 0.0);
}
void BandlimitedImpulseTrainPlotTest::initForLinearFreqSweep(double minFreq, double maxFreq)
{
  rosic::fillWithRangeLinear(inputs[0][0], numFramesToProcess, minFreq, maxFreq);
  rosic::fillWithValue(      inputs[0][1], numFramesToProcess, 0.0);
}
...
{
  rosic::fillWithValue(inputs[0][0], numFramesToProcess, freq);
  rosic::fillWithValue(inputs[0][1], numFramesToProcess, phase);

  rosic::fillWithValue(inputs[0][2], numFramesToProcess,  0.5);
  rosic::fillWithValue(inputs[0][3], numFramesToProcess, -1.0);
...
void SawOscillatorPlotTest::initForFreqSwitch(double freq1, double freq2, int switchTimeInSamples)
{
  ...
  rosic::fillWithValue( inputs[0][0],          length1, freq1);
  rosic::fillWithValue(&inputs[0][0][length1], length2, freq2);
  rosic::fillWithValue(inputs[0][1], numFramesToProcess, 0.0);
}
...
void EnvelopeADSRPlotTest::fillInputSignalArraysWithTestSignal()  
{
  ...
  rosic::fillWithValue(inputs[0][0], numFramesToProcess, attackSeconds);  
  rosic::fillWithValue(inputs[0][1], numFramesToProcess, attackShape); 
  rosic::fillWithValue(inputs[0][2], numFramesToProcess, decaySeconds);  
  rosic::fillWithValue(inputs[0][3], numFramesToProcess, decayShape); 
  rosic::fillWithValue(inputs[0][4], numFramesToProcess, sustainValue);  
  rosic::fillWithValue(inputs[0][5], numFramesToProcess, releaseSeconds);  
  rosic::fillWithValue(inputs[0][6], numFramesToProcess, releaseShape); 
...
void BiquadDesignerPlotTest::initForFreqSweepLowpassBilinear6()
{
  ...
  rosic::fillWithRangeLinear(inputs[0][0], numFramesToProcess, fMin, fMax);
  xAxis = inputs[0][0];
}
...
void BiquadDesignerPlotTest::initForFreqSweepLowpassBilinear12(double q)
{
  ...
  rosic::fillWithRangeLinear(inputs[0][0], numFramesToProcess, fMin, fMax);  
  rosic::fillWithValue(      inputs[0][1], numFramesToProcess, q);  
  xAxis = inputs[0][0];
}
...
void BiquadDesignerPlotTest::initForFreqSweepPeakBilinear(double q, double g)
{
  ...
  rosic::fillWithRangeLinear(inputs[0][0], numFramesToProcess, fMin, fMax);
  rosic::fillWithValue(      inputs[0][1], numFramesToProcess, q);  
  rosic::fillWithValue(      inputs[0][2], numFramesToProcess, g);  
  xAxis = inputs[0][0];
}
