...
namespace romos
{
  ...
  maybe split into AtomicProcessingTests and ContainerProcessingTests
  ...
  */
  ...
  /** A container with 2 inputs and 3 outputs, computing the sum, difference and product of the inputs. If it fails, it may hint that 
  something about containers with multiple I/O pins is broken. */
  ...
  /** Same as SumDiffProdTest but wraps the container that is used there into yet another container. If it fails, it may hint that
  something about container nesting (with multiple I/O pins) is broken. */
  ...
  /** Tests an adder module with dynamic number of inputs (aka summation module). The adder is inside a container with a single input and a 
  single output. It should have 10 connected input pins and one additional disconnected input pin. The test also removes connections to 
  check if the behavior for dynamically deleting pins is as expected. */
  ...
  /** A leaky integrator made from basic modules (unit-delay, multiply, etc.). If this fails, it may hint that something about 
  feedback-paths is broken. */
  ...
  /** A container with a single input and single output and two identity modules in between. The second identity module is placed such that 
  it is evaluated before the first one such that there is an implicit delay in the connection between the two. If this fails, it may hint 
  that something about connections with implicit delay is broken. */
  ...
  /** Similar to LeakyIntegratorTest, but moving one child-module into a position so as to create an additional implicit unit-delay 
  inside the feedback path. */
  ...
  /** A "filter" that computes the sum, difference and product from a LeakyIntegrator and MovingAverage. */
  ...
  /** A module that creates filter impulse response blips at frequencies defined by the note-events that are thrown at it. It uses using 
  a NoteFrequency and a NoteGate module as well as some others. */
  ...
  /** A monophonic ConstantModule value (of 1) that goes into a polyphonic UnaryMinusModule which then goes to the output. Each voice in
  the polyphonic minus-module should receive a copy of the -1 value. And the multiple copies of the -1 should also appear in the 
  output-pins of the container (unless the container itself monophonic in which case it has only one output voice (index 0) which should
  also contain the -1 (because we don't have a voice combiner in front of the output module). 
