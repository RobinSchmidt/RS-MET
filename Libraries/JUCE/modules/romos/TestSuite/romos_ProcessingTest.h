...
namespace romos
{
  ...
  This class is the baseclass for all processing test classes (i.e. test, that produce signals) for the romos library. These tets produce 
  output signals by invoking the 4 processing functions of modules (poly/mono, frame/block-wise) and compare the resulting output signals 
  to known correct output signals. The baseclass provides all the facilities that are needed for such tests, like members which can be used 
  for input- and output- signals, etc.
