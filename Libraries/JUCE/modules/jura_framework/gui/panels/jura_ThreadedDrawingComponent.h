#ifndef jura_ThreadedDrawingComponent_h
#define jura_ThreadedDrawingComponent_h

/** This class is a component, intended to be used as base-class for all components that do some
heavy work when drawing onto their client area. By default, juce calls the Component's paint method 
in the message thread - the same thread where all the user interface interactions (mouse-clicks, 
etc.) take place. Doing heavy graphics rendering in the same thread can make the user interface 
unresponsive. Therefore, this class provides its own infrastructure for handling redrawing: it has 
a member clientAreaImage (pointer to image) onto which subclasses should perform their drawing 
operations - this image will then be shown in paint method. For subclasses of 
ThreadedDrawingComponent, it is typically sufficient to to call setDirty() after each operation 
which invalidates the currently shown image and to override drawComponent() to perform their 
drawing operations on the passed juce::Image. The class ThreadedDrawingComponent together with the 
class ThreadedDrawingComponentDrawingThread will take care of the rest. The passed image will be a 
pointer to our member clientAreaImage and this baseclass here makes sure that drawComponent() is 
called inside a proper CriticalSection to make it thread safe.  */
