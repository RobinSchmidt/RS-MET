#ifndef rojue_InteractiveCoordinateSystem_h
#define rojue_InteractiveCoordinateSystem_h
...
namespace rojue
{
  ...
  This class is a CoordinateSystem with an array CoordinatesystemControlHandles which - in their 
  simplest form - are just points/dots which can be created, dragged around and removed by the 
  mouse. It also provides some functionality which is likely to be useful for subclasses such as 
  a snapToGrid()-method.

  \todo: rename this class (maybe something like SnappingCoordinateSystem)
  maybe factor out the loop-locator stuff into some subclass

  */

  class InteractiveCoordinateSystem	: virtual public CoordinateSystem
  {
  ...
  public:
    ...
    /** Constructor. */
    InteractiveCoordinateSystem(const juce::String& name);   

    /** Destructor. */
    virtual ~InteractiveCoordinateSystem();   
    ...
    virtual XmlElement* getStateAsXml(
      const juce::String& stateName = juce::String(T("InteractiveCoordinateSystemState"))) const;
  ...
  protected:
    ...
    /** Overrides CoordinateSystem::openRightClickPopupMenu() to give more options. */
    ...
    /**< Draws the control-handles on top of the CoordinateSystem which is should 
