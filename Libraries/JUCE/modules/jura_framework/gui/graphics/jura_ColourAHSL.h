#ifndef jura_ColourAHSL_h
#define jura_ColourAHSL_h

/** This class represents a colour via the constituents opacity (alpha), hue, saturation and 
luminance (brightness). This representation is more intuitive to the user than the ARGB (alpha, 
red, green, blue) representation and thus lends itself better to designing color-schemes for 
GUIs. */

class JUCE_API ColourAHSL
{
...
public:
  ...
  /** Constructor. You can pass the initial values for hue, saturation, luminance and alpha here. 
  If you pass nothing, it defaults to opaque black. Hue and saturation will default to 0.f such 
  that when you later increase saturation and luminance, it will become some
  ...
  /** Sets the hue value. Values outside the range 0...1 are periodically wrapped to make a 
  color-circle. */
  ...
  /** Returns a colour with a different hue with respect to this colour (but with all other 
  parameters the same). */
  ...
  /** Returns a colour that has its saturation multiplied with some factor with respect to this 
  colour. Note that when the factor is larger than unity and this colour is already strongly 
  saturated, it might be necesarry to clip the resulting saturation 1.0 */
  ColourAHSL withMultipliedSaturation(float multiplier) const
  {
    return ColourAHSL(hue, multiplier*saturation, luminance, alpha);
  }

  /** Returns a colour that has its luminance raised to some power with respect to this colour. The
  power is commonly referred to as 'gamma' in the image processing community. Above unity, it 
  expands the contrast for bright colors and compresses it for dark colors, below unity, the 
  ...
  /** Returns a colour with several modifiers applied. For the meaning of each, see the 
  corresponding functions for setting one at a
  time. */
  ColourAHSL withModifiersApplied(float hueOffset, float saturationMulitplier, float luminanceGamma)
  {
    return ColourAHSL(hue+hueOffset, saturationMulitplier*saturation, pow(luminance, 
      luminanceGamma), alpha);
  }
