                                   ...
                                   double yd2, double *a3, double *a2, double *a1, double *a0)
{
  ...
  *a0 *= scaler;
  ...
  // maybe precompute x1*x1*x1, x2*x2*x2
}

void rosic::fitCubicWithDerivativeFixedX(double y0, double y1, double yd0, double yd1, double *a3, double *a2, double *a1, double *a0)
{
  // try to optimize - maybe factor out the 3 in the 1st line and the 2 in the second - but make performance tests to check
...
void rosic::getHermiteCoeffsM(double *y0, double *y1, double *a, int M)
{
  ...
  // compute a[0],...,a[M]:
  for(n = 0; n <= M; n++)
    a[n] = y0[n] / factorial(n);

  // establish right hand side (vector k):
  double *k = new double[M+1];  // maybe use alloca - or use a rosic::Vector
  for(n = 0; n <= M; n++)
  {
    k[n] = y1[n];
    for(i = n; i <= M; i++)
      k[n] -= product(i-n+1, i) * a[i];
  }
  ...
  rosic::solveLinearSystem(A.m, &a[M+1], k, M+1);
  delete[] k;
}

void rosic::getHermiteCoeffs1(double *y0, double *y1, double *a)
{
  a[0] = y0[0];     // a0 = y(0)
  a[1] = y0[1];     // a1 = y'(0)
...
void rosic::getHermiteCoeffs2(double *y0, double *y1, double *a)
{
  a[0] = y0[0];     // a0 = y(0)
  a[1] = y0[1];     // a1 = y'(0)
  a[2] = y0[2]/2;   // a2 = y''(0)/2
...
void rosic::getHermiteCoeffs3(double *y0, double *y1, double *a)
{
  a[0] = y0[0];     // a0 = y(0)
  a[1] = y0[1];     // a1 = y'(0)
  a[2] = y0[2]/2;   // a2 = y''(0)/2
...
double rosic::getDelayedSampleAsymmetricHermiteM(double d, double *y, int M, double shape)
{
  int N = 2*M+1;
  int i, j;
                                      ...
                                      double *c, double *d)
{
  ...
  *d = -(x0*(x12*k2+x13*k4+k7*y1)+x02*(x1*(x33*y2-x23*y3)+x13*(x2*y3-x3*y2)+k11*y1)+x03*
  ...
  // ...this seems all very complicated - can this be simplified? ...see formula for Lagrange polynomial in the handout
