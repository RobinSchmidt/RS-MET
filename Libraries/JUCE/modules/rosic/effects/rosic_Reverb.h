...
namespace rosic
{
  ...
  This class implements a feedback-delay-network based reverb with 16 delay-lines. Each of the 16 delaylines has a damping-filter in series 
  which serves as a frequency dependent feedback gain. The feedback itself is realized via a Hadamard-matrix which is implemented via the 
  Fast Hadamard Transform. The injection- and output-vectors are pre-defined. The output-signals of the delay-lines are fed through 
  compensation-filters before actually being routed to the output. These filters decouple the overall reverberation-time from the output 
  volume - and they even do this in a frequency dependent way. Roughly speaking, they act opposite to the damping-filters in order to 
  ...
  */
  ...
  protected:
    ...
    /** Triggers a re-adjustment of one of the read-pointers - this is necesarry when the delay-time of the corresponding delay-line 
    changes. It is also necessarry to do this for all delay-lines when the sample-rate changes. */
    ...
    // These filters have the inverse frequency-response of the damping-filters in the feedback loop and compensate the overall loop-gain 
    // in order to decouple the reverb-time from the loudness of the wet signal. They are used to filter the output-signal:
