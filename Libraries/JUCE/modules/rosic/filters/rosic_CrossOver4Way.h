...
namespace rosic
{
  ...
  */
  ...
  public:
    ...
    /** Sets the crossover frequency for one of the crossovers. The stage parameter refers to the level in the tree (0,1,2) and the
    bandIndex refers to the index within the array of the crossovers in the respective stage. Don't confuse this the output channel number
    (it is true that the index in the final stage corresponds to the same output-channel-index, but that is not true for all stages). */
    ...
    /** Returns the crossover frequency of the filter-pair at the given treeLevel with the given index. */
    ...
    /** Fills the 'magnitudes' array with the magnitude response of one of the filters at the frequencies passed in the 'frequencies'
    array. Both arrays are assumed to be numBins long. */
    void getMagnitudeResponse(double* frequencies, double* magnitudes, int numBins, int outputChannel, bool inDecibels);

    //-------------------------------------------------------------------------------------------------------------------------------------
    // audio processing:

    /** Calculates an output sample frame. The inOut pointer should be a pointer to an array of 16 doubles representing the 8 output-bands
    for left and right in the following order: L1, R2, L2, R2, L3, R3 and so on. The broadband input signal is expected to be passed
    in L1, R1. If the crossover is set up to have less than 4 bands, not all output channels may be used (they will be filled with zeros
    then). */
    INLINE void processSampleFrame(double *inOut);

    /** Processes one buffer of input/output samples. This is a convenience function to make it easier to wrap this class into a plugin.
    The first index is for the channel (and is assumed to range from 0...7) and the second index is the sample-frame number (assumed to
  ...
  INLINE void CrossOver4Way::processSampleFrame(double *inOut)
  {
    double inL = inOut[0];
    double inR = inOut[1];
