...
\todo factor out the aspect of polyphony, provide different modes instead of just the
...
public:
  ...
  void addSlave(LadderFilterOld* newSlave);
...
protected:
  ...
  std::vector<LadderFilterOld*> slaves;
...
INLINE void LadderFilterOld::setCutoff(double newCutoff, bool updateCoefficients)
{
  if(newCutoff != cutoff)
  {
    if(newCutoff < 20.0)
...
INLINE void LadderFilterOld::setResonance(double newResonance, bool updateCoefficients)
{
...
INLINE void LadderFilterOld::calculateCoefficients()
{
...
INLINE double LadderFilterOld::getSample(double in)
{
...
INLINE void LadderFilterOld::getSampleFrameStereo(double *inOutL, double *inOutR)
{
