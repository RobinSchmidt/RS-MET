...
namespace rosic
{
  ...
  */
  ...
  public:
    ...
    /** Sets the current key and velocity, so the oscillator can set up it's key and velocity dependent parameters. */
    void setKeyAndVel(int newKey, int newVel);

    /** Sets the nominal frequency to be played. The actual playback-frequency will additionally take into account the detune-factor as set 
    by setDetune(). */
    ...
    */
    ...
    /** Sets the phase-shift of the the harmonics between left and right channel 8in degrees. The left channel's harmoincs will be shifted 
    half this value into the negative direction and the right channel's harmonics by the same amount into the positive direction. */
    void setStereoPhaseShift(double newPhaseShift);

    /** Sets an additional phase-shift between even and odd harmonics, half of which is applied with negative sign to the left channel and 
    with positive sign to the right channel. */
    ...
    /** Fills the targetBuffer with values suitable for displaying the current waveform in a display. The targetBuffer is assumed to be of 
    size [2][numSamplesToShow] where the first index represents the channel and the second represents the sample-number on the display 
    (which is the x-coordinate in pixels). */
    ...
    /** Calculates the phase increment from the nominal frequency and the detune-factor for this instance and also triggers the increment 
    calculation for all slaves. */
    ...
    /** Adds a  slave instance to the vector of slaves - this will also cause the 'isMaster' flag of the slave to be set to false, redirect 
    the slaves parameters-pointer to the one of this instance and delete the old (now unused) parameters pointer of the slave. */
    void addSlave(OscillatorStereo* newSlave);

    //-------------------------------------------------------------------------------------------------------------------------------------
    // embedded publically accessible objects:

    /** This is a pointer to the wavetable object to be used. We use a pointer instead of a direct member object in order to share the same 
    wavetable among a bunch of oscillators - the idea is, that an oscillator which is part of a synthesizer-voice can share the wavetable 
    with its siblings of the other voices. */
