...
namespace rosic
{
  ...
  */
  ...
  class Quadriga : public PolyphonicInstrument
  {
  ...
  protected:
    ...
    MipMappedWaveTableStereo xLfoWaveTable, yLfoWaveTable;
    */
  ...
  INLINE void Quadriga::getSampleFrameStereo(double *outL, double *outR)
  {
    if( voicePointers == NULL )
      return;

    // cast the voicePointer array to the subclass:
    QuadrigaVoice* castedVoicePointer;
    ...
    for(i=1; i<=numPlayableVoices; i++)
    {
      ...
      {
        castedVoicePointer = static_cast<QuadrigaVoice*>(voicePointers[i]);
