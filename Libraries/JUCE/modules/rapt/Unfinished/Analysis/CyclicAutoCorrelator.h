#ifndef RS_CYCLICAUTOCORRELATOR_H
#define RS_CYCLICAUTOCORRELATOR_H
...
namespace RSLib
{
  ...
  */
  ...
  class RSLib_API rsCyclicAutoCorrelator  
  {
  ...
  public:
    ...
    RS_INLINE void acceptSample(double in);

    /** Calculates the autocorrelation between the past two pitch-cycles. */
    RS_INLINE double getCorrelationAndReset();
  ...
  protected:
    ...
    double sumOfSquares;  // Accumulates the squares of the input samples.
    double oldMeanSquare; // mean-square value of the old pitch-period
    double *buffer;       // stores the previous pitch-period
  ...
  RS_INLINE void rsCyclicAutoCorrelator::acceptSample(double in)
  {
  ...
  RS_INLINE double rsCyclicAutoCorrelator::getCorrelationAndReset()
  {
    // determine the number of samples over which we have to average:
    double numSamplesToAverage = (double) rsMin(sampleCounter, oldCycleLength);
    ...
    double normalizer  = rsSqrt(oldMeanSquare * meanSquare);
