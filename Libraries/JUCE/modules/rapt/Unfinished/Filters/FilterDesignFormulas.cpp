namespace RSLib
{
  ...
  void invertBiquad(double &b0, double &b1, double &b2, double &a1, double &a2)
  {
    // normalize numerator:
    double g = b0;
    double s = 1/g;
  ...
  void twoOnePolesToBiquad(double b0[2], double b1[2], double a1[2],                   
  ...
  void ensureOnePoleRootInsideUnitCircle(double &c0, double &c1)
  {
    double root = -c1/c0;
    if( fabs(root) > 1.0 )
    {
  ...
  void intermediatesToOnePoleCoeffs(double C0, double C1, double &c0, double &c1)
  {
  ...
  void magnitudeMatchedOnePoleCoeffs(double &b0, double &b1, double &a1, 
    double w[3], double m[3])
  {
    ...
    double pOld = (b0*b0+b1*b1)/(1+a1*a1);
    ...
    double pNew = (b0*b0+b1*b1)/(1+a1*a1);
    s = rsSqrt(pOld/pNew);
    b0 *= s;
    b1 *= s;
