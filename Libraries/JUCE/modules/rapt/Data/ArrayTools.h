...
public:
  ...
  template <class T>
  static void rsAdd(T *buffer1, T *buffer2, T *result, int length);
  ...
  template <class T>
  static void rsAdd(T *buffer, T valueToAdd, T *result, int length);
  ...
  template <class T>
  static void rsAddCircularShiftedCopy(T *buffer, int length, double offset, T weight);

  // add length-L array y into length-N array x starting at n
  template<class T>
  static void rsAddInto(T *x, int N, T *y, int L, int n = 0);
  ...
  template<class T>
  static void rsAllocateSquareArray2D(T**& theArray, int size);
  ...
  template <class T>
  static void rsApplyFunction(T *inBuffer, T *outBuffer, int length, T (*f) (T));

  /** Checks, if the two buffers are elementwise approximately equal within the given tolerance. */
  template <class T>
  static bool rsAreBuffersApproximatelyEqual(T *buffer1, T *buffer2, int length, T tolerance);

  /** Checks, if the two buffers are elementwise equal. */
  template <class T>
  static bool rsAreBuffersEqual(T *buffer1, T *buffer2, int length);
  ...
  template<class T>
  static void rsCircularShift(T *buffer, int length, int numPositions);
  ...
  the buffer. */
  template <class T>
  static void rsCircularShiftInterpolated(T *buffer, int length, double numPositions);

  /** Restricts the values in the buffer to the range between min and max for types that define the
  operators '<' and '>'. */
  template <class T>
  static void rsClipBuffer(T *buffer, int length, T min, T max);
  ...
  system. */
  template <class T>
  static int rsCompare(T *a, T *b, int length);
  ...
  to be greater. */
  template <class T>
  static int rsCompare(T *a, int na, T *b, int nb);

  /** Searches the array for an element (via the '==' operator of type T) and returns true if the
  element was found. */
  template <class T>
  static bool rsContains(T *buffer, int length, T elementToFind);
  ...
  template <class T>
  static void rsConvolve(T *x, int xLength, T *h, int hLength, T *y);

  /** Copies the data of one array into another one and converts the type if necessary. */
  template <class T1, class T2>
  static void rsConvertBuffer(T1 *source, T2 *destination, int length);
  ...
  template <class T>
  static void rsConvolveInPlace(T *x, int xLength, T *h, int hLength);
  ...
  template <class T1, class T2>
  static void rsCopyBuffer(const T1 *source, T2 *destination, int length);
  ...
  garbage. */
  template <class T>
  static int rsCopyIfMatching(T *sourceBuffer, T *targetBuffer, int sourceAndTargetLength,
  ...
  template <class T>
  static int rsCopyIfNotMatching(T *sourceBuffer, T *targetBuffer, int sourceAndTargetLength,
  ...
  template <class T>
  static void rsCopyBufferWithLinearInterpolation(T *source, int sourceLength, T *destination,
  ...
  template<class T1, class T2>
  static void rsCopySection(T1 *source, int sourceLength, T2 *destination, int copyStart, int copyLength);
  ...
  */
  ...
  template <class T>
  static void rsCumulativeSum(T *x, T *y, int N);

  /** Computes a cumulative sum of arbirtry order of x and stores it in y. Can be used in place. */
  template <class T>
  static void rsCumulativeSum(T *x, T *y, int N, int order);

  /** Frees memory allocated previously via rsAllocateSquareArray2D. */
  template<class T>
  static void rsDeAllocateSquareArray2D(T**& theArray, int size);
  ...
  template <class T>
  static void rsDeConvolve(T *y, int yLength, T *h, int hLength, T *x);

  /** De-interleaves a buffer of interleaved data. @see rsInterleave */
  template <class T>
  static void rsDeInterleave(T *buffer, int numFrames, int numElementsPerFrame);
  ...
  template <class T>
  static void rsDifference(T *buffer, int length, int order = 1, bool periodic = false);
  ...
  static void rsDivide(T1 *buffer1, T2 *buffer2, TR *result, int length);
  ...
  template <class T>
  static void rsFillWithIndex(T *buffer, int length);

  /** Fills the buffer with random values between min and max. */
  template <class T>
  static void rsFillWithRandomValues(T *buffer, int length, double min, double max, int seed);

  /** Fills the buffer with values ranging (exponentially scaled) from min to max (both end-values
  inclusive). */
  template <class T>
  static void rsFillWithRangeExponential(T *buffer, int length, T min, T max);

  /** Fills the buffer with values ranging from min to max (both end-values inclusive). 
  \todo: rename min/max into start/end  */
  template <class T>
  static void rsFillWithRangeLinear(T *buffer, int length, T min, T max);

  /** Fills the passed array with one value at all indices. */
  template <class T>
  static void rsFillWithValue(T *buffer, int length, T value);
  ...
  template <class T>
  static void rsFillWithZeros(T *buffer, int length);
  ...
  template <class T>
  static void rsFilter(T *x, int xLength, T *y, int yLength, T *b, int bOrder, T *a, int aOrder);

  /** \todo check and comment this function - maybe move it to RSLib */
  template <class T>
  static void rsFilterBiDirectional(T *x, int xLength, T *y, int yLength, T *b, int bOrder, T *a,
  ...
  none is found. */
  template<class T>
  static int rsFindPeakOrValleyRight(T *x, int N, int n0);

  /** Returns the index of a peak or value in the length-N array x left or equal to n0 or -1 if 
  none is found. */
  template<class T>
  static int rsFindPeakOrValleyLeft(T *x, int N, int n0);
  ...
  */
  template <class T>
  static int rsFirstIndexWithNonZeroValue(T *buffer, int length);

  /** Scales and offsets the passed buffer such that the minimum value hits 'min' and the
  maximum value hits 'max'. */
  template <class T>
  static void rsFitIntoRange(T *buffer, int length, T min, T max);
  ...
  template <class T>
  static void rsImpulseResponse(T *h, int hLength, T *b, int bOrder, T *a, int aOrder);

  /** Interleaves a buffer of non-interleaved data. */
  template <class T>
  static void rsInterleave(T *buffer, int numFrames, int numElementsPerFrame);
  ...
  template <class T>
  static void rsNormalize(T *buffer, int length, T maximum, bool subtractMean = false);
  ...
  template <class T>
  static void rsOrderBitReversed(T *buffer, int N, int log2N);
  ...
  template <class T>
  static void rsOrderBitReversedOutOfPlace(T *inBuffer, T *outBuffer, int length, int numBits);
  ...
  is passed . */
  template <class T>
  static T rsProduct(const T* const buffer, int length);
  ...
  static void rsSwapDataBuffers(void *buffer1, void *buffer2, void *bufferTmp, int sizeInBytes);
  ...
  template<class T>
  static void rsTransposeSquareArray(T **in, T **out, int size);

  /** In-place version of rsTransposeSquareArray(T **in, T **out, int size). */
  template<class T>
  static void rsTransposeSquareArray(T **theArray, int size);
  // move to MatrixFunctions

  /** Returns the sum-over-i w[i]*x[i]. */
  template <class T>
  static T rsWeightedSum(T *w, T *x, rsUint32 length);

  /** Forms a weighted sum of the two buffers. */
  template <class T>
  static  void rsWeightedSum(T *buffer1, T *buffer2, T *result, int length, T weight1, T weight2);
