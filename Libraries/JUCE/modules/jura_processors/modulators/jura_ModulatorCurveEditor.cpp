...
void ModulatorCurveEditor::mouseDrag(const MouseEvent &e)
{
    ...
    breakpointBeingDragged <= modulatorToEdit->lastBreakpointIndex() )
  {
    ...
    if( e.mods.isShiftDown() )
      modulatorToEdit->setEditMode(rosic::BreakpointModulator::EDIT_WITH_SHIFT);
    else
      modulatorToEdit->setEditMode(rosic::BreakpointModulator::EDIT_WITHOUT_SHIFT);
...
void ModulatorCurveEditor::updatePlotCurveData(int curveIndex, 
  rosic::BreakpointModulator* modulator, bool updateGUI)
{
  if( modulator == NULL )
    return;
...
void ModulatorCurveEditor::plotLoopLocators(Graphics &g, juce::Image *targetImage, 
  rosic::BreakpointModulator* modulator, const Colour& locatorColour, bool fullHeight)
{
  if( modulator == NULL )
    return;
