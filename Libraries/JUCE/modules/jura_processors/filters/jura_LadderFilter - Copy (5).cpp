...
void Ladder::createStaticParameters()
{
  ...
  p = new AutomatableParameter(plugInLock, "Resonance", 0.0, 1.0, 0.0, 0.0, 
    Parameter::LINEAR, 71);
  addObservedParameter(p);
  ...
  XmlElement *targetSVG)
{
  ...
  y = (float) resoToY( filterToEdit->getResonance(), targetImage);
    ...
    // maybe, we should inquire the values of cutoff not from the filter but form our parameter 
    // objects - that would be cleaner

  x = freqParameter->getValue();   // we need to check against nullptr
  y = resoParameter->getValue();   // we need to check against nullptr
...
{
  double maxRes = 1.0;   // preliminary - should be stored somewhere else
  return (1.0 - reso/maxRes) * getPlotHeight(targetImage);
...
{
  double maxRes = 1.0;   // preliminary - should be stored somewhere else
  return maxRes * ( 1.0 - y/getPlotHeight(targetImage) );  
