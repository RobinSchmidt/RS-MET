#ifndef rojue_RComboBox_h
#define rojue_RComboBox_h
...
namespace rojue
{
  ...
  //=======================================================================================================================================
  // the actual combobox class (and associated oberserver class):

  class RComboBox;

  /** Observer class for RComboBoxes */

  class RComboBoxObserver
  {
  ...
  A component that lets the user choose from a drop-down list of choices. The combo-box has a list of text strings, each with an associated 
  id number, that will be shown in the drop-down list when the user clicks on the component. The currently selected choice is displayed in 
  the combo-box, and this can either be read-only text, or editable. To find out when the user selects a different item or edits the text, 
  you can register a RComboBoxListener to receive callbacks. @see RComboBoxListener

  */

  class RComboBox : public RTextField, public RPopUpMenuObserver, public RPopUpOwner
  {
  ...
  public:
    ...
    /** Creates a combo-box. On construction, the text field will be empty, so you should call the setSelectedId() or setText() method to 
    choose the initial value before displaying it. */
    RComboBox (const juce::String& componentName);
    ...
    virtual void addItem(int itemResultId, const juce::String& itemText, bool isEnabled = true, bool isTicked = false);

    /** This allows items in the drop-down list to be selectively disabled. When you add an item, it's enabled by default, but you can call 
    this method to change its status. If you disable an item which is already selected, this won't change the current selection - it just 
    stops the user choosing that item from the list. */
    ...
    /** Removes all the items from the drop-down list. If this call causes the content to be cleared, then a change-message will be 
    broadcast unless dontSendChangeMessage is true. @see addItem, removeItem, getNumItems */
    ...
    /** Assuming that the passed text matches one of the item-texts, this function sets the contents of the combo-box's text field to the 
    respective item. If there is no matching item, a debug-break will be issued. */
    ...
    /** Overrides a RWidget::assignParameter in order to set up the available options to those that are defined in the Parameter. */
    ...
    /** Returns the text for one of the items in the list. Note that this doesn't include headers or separators. @param index  the item's 
    ...
    /** Returns the text of the selected item if any, otherwise "nothing selected" will be returned . */
    virtual juce::String getSelectedItemText() const;

    /** Overriden from RWidget - returns the state (defined as the currently selected text) as string. */
  ...
  \todo: override setDescriptionField, colour-scheme etc. to pass the values through to the child widgets
   ...or maybe don't let this class have child-components (i.e. the RTextField) - instead, handle drawing differently in paint
  
  */

  class RNamedComboBox : public RComboBox
  {
  ...
  public:
    ...
    /** Sets the name of the combobox - this is the text that appears in front of (or above) the actual combobox. */
  ...
  private:
    ...
    /** Moved to private area, because it should not be used for this subclass. Instead, the two-parameteric form of this function should 
    be used. */
    //virtual void setColourScheme(const WidgetColourScheme& newColourScheme) {}
