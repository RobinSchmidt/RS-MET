...
int RTextField::getTextPixelPositionX() const
{
  ...
  if( justification.testFlags(hFlags & Justification::centredLeft) )
    ...
    x = getWidth()/2 - boldFont10px.getTextPixelWidth(text, boldFont10px.getDefaultKerning())/2;
  else if( justification.testFlags(hFlags & Justification::centredRight) )
    x = getWidth() - boldFont10px.getTextPixelWidth(text, boldFont10px.getDefaultKerning()) - horizontalMargin;
...
{
  return getHeight()/2 - boldFont10px.getFontAscent()/2;
}

int RTextField::getWidthToFitText() const
{
  return boldFont10px.getTextPixelWidth(text, boldFont10px.getDefaultKerning()) + 2*outlineThickness + 2*horizontalMargin;
}
...
bool RTextEntryField::keyPressed(const KeyPress &key)
{
  ...
  {
    if( key == KeyPress(T('c'), ModifierKeys::commandModifier, 0) )
    {     
      if( !isSelectionEmpty() )
