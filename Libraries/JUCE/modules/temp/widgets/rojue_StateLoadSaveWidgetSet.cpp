#include "rojue_StateLoadSaveWidgetSet.h"
...
{
  addWidget( stateLabel = new RTextField( String(T("State:"))) );
  stateLabel->setDescription(String(T("Name of current file (if any)")));
  stateLabel->setNoBackgroundAndOutline(true);

  addWidget(stateFileNameLabel = new RTextField(String::empty) );
  stateFileNameLabel->setDescription(String(T("Name of current file (if any)")));
  ...
  stateLoadButton->setDescription(String(T("Load setting from file")));
  ...
  stateSaveButton->setDescription(String(T("Save current setting to file")));
  ...
  stateMinusButton->setDescription(String(T("Skip to previous file in current directory")));
  ...
  statePlusButton->setDescription(String(T("Skip to next file in current directory")));
...
void StateLoadSaveWidgetSet::rButtonClicked(RButton *buttonThatWasClicked)
{
  ...
  StateFileManager* underlyingStateFileManager = dynamic_cast<StateFileManager*> (watchedStateManager);
