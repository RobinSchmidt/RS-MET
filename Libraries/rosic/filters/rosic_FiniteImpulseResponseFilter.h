...
namespace rosic
{
  ...
  This class implements a filter with finite impulse response.It uses a partitioned fast convolution algorithm to perform the filtering.
  ...
  */
  ...
  public:
    ...
    bandreject and peak filters the center frequency. */
    ...
    // \todo setMagnitudeResponse - create a linear-phase FIR filter with the desired magnitude response
    // \todo maybe introduce a flag to the setters to avoid instant update of the coeffs - facilitates avoidance multiple re-calculations
    //       when several parameters change at once
