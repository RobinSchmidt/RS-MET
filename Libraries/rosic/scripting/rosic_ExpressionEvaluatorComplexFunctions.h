...
public:
  ...
  double DoEvaluate()
  {
    Complex z  = Complex(m_nodes[0]->Evaluate(), m_nodes[1]->Evaluate());
    Complex w  = acosC(z);
    *m_refs[0] = w.re;
    *m_refs[1] = w.im;
    return 0.0;
...
public:
  ...
  double DoEvaluate()
  {
    ...
    sinCos(m_nodes[1]->Evaluate(), m_refs[1], m_refs[0]);
    *m_refs[0] *= tmp;
    *m_refs[1] *= tmp;
    return 0.0;
  }
...
public:
  ...
  double DoEvaluate()
  {
    Complex basis    = Complex(m_nodes[0]->Evaluate(), m_nodes[1]->Evaluate());
    Complex exponent = Complex(m_nodes[2]->Evaluate(), m_nodes[3]->Evaluate());
    Complex result   = powC(basis, exponent);
    *m_refs[0]       = result.re;
    *m_refs[1]       = result.im;
    return 0.0;
  }
