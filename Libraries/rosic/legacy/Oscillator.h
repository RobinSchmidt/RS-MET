...
This is a table-lookup oscillator with linear interpolation. There are several
standard waveforms as well as white noise and pink noise (therefore a 
White2PinkFilter object is embedded) and a lot of special waveforms. The 
output samples can be generated by the simple getSample()-function or by
the more elaborate getSampleAntiAliased()-function.
...
public:
 ...
 /**< Calculates a single output-sample by means of linear interpolation.
      This is the LoFi-version because no anti-aliasing algorithm is used.
      For better quality use the getSampleAntiAliased() method - but this, of
      course, requires more CPU. */

 INLINE double getSample();  
 /**< Calculates an anti-aliased output sample. The anti aliasing is achieved 
      via a multi-sampled table, where each of the successive tables halves 
      the spectral content. To choose, which table is to be used, the exponent
      of the phase-increment is extracted - .... 
      costs: bypass:       20
             white noise: 101  
             pink noise:  166
             waveform:    132     */
 ...
 //---------------------------------------------------------------------------
 // parameter settings that are supposed to be updated at sample-rate 
 ...
 /**< An object of class WaveTable should be passed with this function which
´     will be used in the oscillator. Not to have "our own" WaveTable-object 
      as member-variable avoids the need to have the same waveform for 
      different synth-voices multiple times in the memory. */
 ...
 /**< Reset the phaseIndex to startIndex+initialPhaseAdvance and resets the 
      blepPostion to ... */
...
protected:
 ...
 static doubleA blep[8193]; // an array containing a mininmum-phase 
                            // bandlimited-step ('minBlep')  
 ...
 WaveTable*         waveTable; // a pointer to a wavetable-object
...
INLINE void Oscillator::setFreq(double newFreq)
{
...
INLINE void Oscillator::calcIncrements()
{
