...
namespace rosic
{
  ...
  PolyphonicInstrument is a base class from which polyphonic instruments can be derived. One should derive a single voice for an 
  instrument from the PolyphonicIntrumentVoice class and the whole polyphonic instrument then from this class here. These two 
  classes will then handle the voice management.

  introduce public flag allVoicesAreSilent 
  ...
  */
  ...
  public:
    ...
    /** Constructor. Sets the variable numAllocatedVoices - however: the actual allocation of the voices MUST be done in your subclass! */
    ...
    /** Sets the maximum number of playable voices (polyphony) - the zeroth voice is supposed to serve as master/template voice and is not 
    playable. */
    ...
    /** Sets up an automatic scaling of the master level by the number of playing voices in order to decouple the overall loudness from 
    that. */  
    ...
    /** Switches the note-glide function on or off - when on, this object will call glideTo() instead of noteOn() for the the stolen voice. 
    The glideTo()-function (virtual memeber of the PolyphonicInstrumentVoice class) is supposed to cause a glide to a new pitch instead of 
    re-triggering the voice. When number of voices is 1, this works as the usual glide in monophonic synths. */
    virtual void setGlideMode(bool shouldUseGlide);

    /** Calls setBeatsPerMinute for all the voices - in PolyphonicInstrumentVoice, this function is empty, you will need to override it in 
    your subclass if you need sync functionality. */
    ...
    /** Triggers a note. Depending on the current setting of polyphony, voice assignment, etc. it will choose a voice and call the 
    noteOn()-function of the chosen voice. */
    ...
    /** Overrides the setMidiController function inherited from AutomatableModule in order to treat controller 64 (sustain) differently. */
  ...
  protected:
    ...
    /** Applies all the volume scalings such as masterVolume, the cumulative power compensation and mid/side adjustment. */
    INLINE void applyVolumeScalers(double *left, double *right, double cumulativePower);

    /** Calculates and returns a scaling factor for the output signal which compensates (to an user adjustable extent) the gain in ouput 
    power when several voices are playing together - the input argument should be the cumulative power of all playing voices. A reaonable 
    way to obtain this value could be to sum the squares of the amplitude envelopes of the individual voices. */
    ...
    /** This function is called when the sustain switch is turned off - it sends out all the note offs which have been collected during 
    sustain was on and resets the array of pending note-offs to all false. */
    void dischargePendingNoteOffs();

    /** This is an array of pointers to the actual voices which will be in general of a subclass of PolyphonicInstrumentVoice - but we use 
    only the base class pointer here. The respective subclass of PolyphonicInstrument must take care to properly assign these pointers in 
    its constructor such that they point to the actual objects which realize those voices. The voice with index 0 plays a special role - it 
    will serve as master or template for all the others and should be accessed by the GUI. It will not be used for actual audio 
    processing. */
    PolyphonicInstrumentVoice** voicePointers;

    /** Number of allocated voices - this must be set in the constructor. The number of playable voices will be one less - the zeroth voice 
    serves only as template for the others. */
    int numAllocatedVoices;

    /** The current number of playable voices - this can be set by setNumPlayableVoices to at most numAllocatedVoices-1 . */
    ...
    /** Mid/side ratio and scaling factors for mid and side signal. You may or may not use them in getSampleFrameStereo in your 
    subclass. */
  ...
  INLINE void PolyphonicInstrument::applyVolumeScalers(double *left, double *right, double cumulativePower)
  {
  ...
  {
    double psq = max(1.0, cumulativePower);
  ...
  INLINE void PolyphonicInstrument::getSampleFrameStereo(double *outL, double *outR)
  {
    ...
    // loop through the voices, voicePointers[0] servers only as template for the others and is not invoked to process audio:
