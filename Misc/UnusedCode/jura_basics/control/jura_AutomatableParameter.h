...
-maybe move to file jura_Parameter.h/cpp
...
public:
  ...
  virtual void setValue(double newValue, bool sendNotification)
  {
    ...
    Parameter::setValue(newValue, sendNotification);
  ...
  successful - if not, it means that the desired (new) lower limit was higher than the current 
  (old) upper limit. */
  ...
  successful - if not, it means that the desired (new) upper limit was lower than the current (old) 
  lower limit. */
  ...
  parameter, and if so, it sets up the
  automation value to controllerValue / 127.0. */
  virtual void setMidiController(int controllerNumber, int controllerValue);

  /** Switches this parameter into midi-learn mode - the very next received MIDI controller will 
  be assigned to this parameter then. */
  ...
  value - if this second is done (first parameter true), there is a further option to do this with 
  or without sending a notification to the listeners. */
  //virtual void revertToDefaults(bool resetValueAlso = false, bool sendNotification = false);
  virtual void revertToDefaults(bool resetValueAlso, bool sendNotification);
 ...
 /** Returns the default MIDI controller assigned to this parameter, default: -1 (none). */
  ...
  /** Informs whether or not this parameter is in default state, that is: are the assigned lower and 
  upper limits and the assigned controller still in their default settings. */
