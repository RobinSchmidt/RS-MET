#include "rosof_PolyphonicInstrumentAudioModule.h"
...
void PolyphonicInstrumentAudioModule::setStateFromXml(const XmlElement& xmlState, const juce::String& stateName, bool markAsClean) 
{
...
void PolyphonicInstrumentAudioModule::initializeAutomatableParameters()
{
  ...
  // #000:
  p = new AutomatableParameter(plugInLock, "MasterLevel", -36.0, 12.0, 0.1, 0.0, Parameter::LINEAR, 7);
  addObservedParameter(p);

  // #001:
  p = new AutomatableParameter(plugInLock, "VoiceLevelByKey", -24.0, 24.0, 0.1, 0.0, Parameter::LINEAR);
  addObservedParameter(p);

  // #002:
  p = new AutomatableParameter(plugInLock, "VoiceLevelByVel", 0.0, 12.0, 0.1, 0.0, Parameter::LINEAR);
  addObservedParameter(p);

  // #003:
  p = new AutomatableParameter(plugInLock, "MasterLevelByVoices", 0.0, 100.0, 0.1, 0.0, Parameter::LINEAR);
  addObservedParameter(p);
  ...
  // #005:
  p = new AutomatableParameter(plugInLock, "GlideSwitch", 0.0, 1.0, 0.0, 0.0, Parameter::BOOLEAN);
  addObservedParameter(p);

  // #006:
  p = new AutomatableParameter(plugInLock, "GlideTime", 5.0, 2000.0, 0.0, 50.0, Parameter::EXPONENTIAL);
  addObservedParameter(p);

  // #007:
  p = new AutomatableParameter(plugInLock, "MasterTuneA4", 220.0, 880.0, 0.01, 440.0, Parameter::EXPONENTIAL);
  addObservedParameter(p);

  // #008:
  p = new AutomatableParameter(plugInLock, "PitchWheelRange", 0.0, 24.0, 0.1, 12.0, Parameter::LINEAR);
  addObservedParameter(p);
