#ifndef rojue_ColourScheme_h
#define rojue_ColourScheme_h
...
namespace rojue
{
  ...
  This is a baseclass fo ColourSchemes for various GUI objects - like, for example, widgets, plots, editors, etc. The basclass defines some 
  macro-parameters that can be applied to all these objects such as the gnereal look (bright-on-dar vs. dark on bright), a hue offset, a 
  saturation multiplier, etc. Whe the user changes one of these macro parameters, a call to updateColours() will be triggered - you need to
  override this function in your subclass to compute the actual colours.
  ...
  */
  ...
  public:
    ...
    virtual void setSaturationMultiplier(float newMultiplier) { saturationMultiplier = newMultiplier; updateColours(); }
    ...
    /** Appends a hue-offset into our array of hue-offsets (which are used for various parts of the editor). */
    virtual void appendHueOffset(float offsetToAppend) { hueOffsets.add(offsetToAppend); }

    /** Sets up one of the hue-offsets with the given index. If the given index does not yet exist in our array, the new offset will be 
    appended, possibly filling up with zeros at indices in between the current max-index and the new index. */
    ...
    /** Returns the number of defined hue-offsets. These offsets may be used for various parts of the editor or sub-editors. */
    virtual int getNumHueOffsets() const { return hueOffsets.size(); }

    /** Returns the hue-offset at the given index. These offsets may be used for various parts of the editor or sub-editors. */
  ...
  protected:
    ...
    /** Method that will be called when the user has changed one of our macro-parameters - subclasses should update their colours then 
    accordingly. */
  ...
  class EditorColourScheme : public ColourScheme
  {
  ...
  class WidgetColourScheme : public ColourScheme
  {
  ...
  class PlotColourScheme : public ColourScheme
  {
  ...
  public:
    ...
    /** Selects one of the strategies to colour multiple curves in the plot. This will determine what Colour will be returned by subsequent
    call to getCurveColour @see curveColouringStrategies */
