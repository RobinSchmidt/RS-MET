...
{
  if( newAppearanceString == String(T("DarkOnBright")) )
    setAppearance(DARK_ON_BRIGHT);
  else if( newAppearanceString == String(T("BrightOnDark")) ) 
...
juce::String ColourScheme::getAppearanceString() const
{
  switch( appearance )
  {
  case DARK_ON_BRIGHT: return String(T("DarkOnBright"));
  case BRIGHT_ON_DARK: return String(T("BrightOnDark"));
...
{    
  ColourAHSL topLeftAHSL, topRightAHSL, bottomLeftAHSL, bottomRightAHSL, outlineAHSL, textAHSL, axesAHSL, coarseGridAHSL, 
    fineGridAHSL; //, curvesAHSL;
