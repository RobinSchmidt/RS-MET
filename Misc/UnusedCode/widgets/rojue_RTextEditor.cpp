...
#define SHOULD_WRAP(x, wrapwidth)       (((x) - 0.0001f) >= (wrapwidth))
  ...
  public:
    ...
    UniformTextSection(const String& text, BitmapFont const* font_, const Colour& colour_) throw() : font (font_), colour (colour_)
    {
      initialiseAtoms(text);
    }

    UniformTextSection (const UniformTextSection& other) throw() : font (other.font), colour (other.colour)
    {
        ...
        if(lastAtom != 0)
        {
          if( ! CharacterFunctions::isWhitespace(lastAtom->atomText.getLastCharacter()) )
          {
            ...
            if( ! CharacterFunctions::isWhitespace(first->atomText[0]) )
            {
              ...
              lastAtom->width = font->getTextPixelWidth(lastAtom->getText(), font->getDefaultKerning());
              delete first;
              ++i;
            }
          }
    ...
    UniformTextSection* split(const int indexToBreakAt) throw()
    {
      ...
      for(int i=0; i<atoms.size(); ++i)
      {
        ...
        else if(indexToBreakAt >= index && indexToBreakAt < nextIndex)
        {
          ...
          secondAtom->width    = font->getTextPixelWidth(secondAtom->getText(), font->getDefaultKerning());
  ...
  private:
    ...
    void initialiseAtoms(const String& textToParse) throw()
    {
      ...
      while(i < len)
      {
        ...
        if( CharacterFunctions::isWhitespace(text[i]) && text[i] != T('\r') && text[i] != T('\n') )
        {
  ...
  public:
    ...
    RTextEditorIterator(const Array<void*>& sections_, const int wordWrapWidth_) throw() : indexInText(0), lineY(0), lineHeight(0), 
      maxDescent(0), atomX(0), atomRight(0), atom(0), currentSection(0), sections(sections_), sectionIndex(0), atomIndex(0),
    ...
    RTextEditorIterator (const RTextEditorIterator& other) throw() : indexInText(other.indexInText), lineY(other.lineY), 
      lineHeight(other.lineHeight), maxDescent(other.maxDescent), atomX(other.atomX), atomRight(other.atomRight), atom(other.atom),
      currentSection(other.currentSection), sections(other.sections), sectionIndex(other.sectionIndex), atomIndex(other.atomIndex),
    ...
    bool next() throw()
    {
      ...
      else if(atomIndex >= currentSection->getNumAtoms() - 1)
      {
        ...
        else
        {
          ...
          {
            // handle the case where the last atom in a section is actually part of the same word as the first atom of the next section:
      ...
      if(SHOULD_WRAP(atomRight, wordWrapWidth) || forceNewLine)
      {
        if(atom->isWhitespace())
          atomRight = jmin (atomRight, wordWrapWidth); // leave whitespace at the end of a line, but truncate it to avoid scrolling
        else
    ...
    void draw(Graphics& g, const UniformTextSection*& lastSection) const throw()
    {
      if( !atom->isWhitespace() )
      {
        ...
        drawBitmapFontText(g, atomX, lineY, atom->getTrimmedText(), (currentSection->font), currentSection->colour);
      }
    }
    ...
    void drawSelectedText(Graphics& g, const int selectionStart, const int selectionEnd, const Colour& selectedTextColour) const throw()
    {
      if( !atom->isWhitespace() )
      {
        /*
        ...
        */
        ...
        drawBitmapFontText(g, atomX, lineY, atom->getTrimmedText(), currentSection->font, selectedTextColour);
    ...
    int indexToX (const int indexToFind) const throw()
    {
      ...
      if(indexToFind >= indexInText + atom->numChars)
        return atomRight;
      return currentSection->font->glyphIndexToX(atom->getText(), indexToFind-indexInText, currentSection->font->getDefaultKerning());
    }
    ...
    int xToIndex(const int xToFind) const throw()
    {
      ...
      return currentSection->font->xToGlyphIndex(atom->getText(), xToFind, currentSection->font->getDefaultKerning());
    }
  ...
  public:
    ...
    RTextEditorInsertAction(RTextEditor& owner_, const String& text_, const int insertIndex_, BitmapFont const* font_, 
      const Colour& colour_, const int oldCaretPos_, const int newCaretPos_) throw() 
      : owner(owner_), text(text_), insertIndex(insertIndex_), oldCaretPos(oldCaretPos_), newCaretPos(newCaretPos_), font(font_),
      colour (colour_)
    {
  ...
  public:
    ...
    RTextEditorRemoveAction(RTextEditor& owner_, const int startIndex_, const int endIndex_, const int oldCaretPos_, 
      const int newCaretPos_, const Array<void*>& removedSections_) throw() : owner(owner_),  startIndex(startIndex_), endIndex(endIndex_),
      oldCaretPos(oldCaretPos_), newCaretPos(newCaretPos_), removedSections(removedSections_)
    {
  ...
  void RTextEditor::setText(const String& newText, const bool sendTextChangeMessage)
  {
    ...
    if(newLength != getTotalNumChars() || getText() != newText)
    {
      ...
      // if you're adding text with line-feeds to a single-line text editor, it ain't gonna look right!
  ...
  void RTextEditor::updateTextHolderSize() throw()
  {
    ...
    if(wordWrapWidth > 0)
    {
      ...
      const int h = topIndent  + jmax(i.lineY+i.lineHeight, currentFont->getFontHeight()+textEditorLineSpacing);
  ...
  void RTextEditor::setBorder(const BorderSize& border) throw()
  {
    borderSize = border;
    resized();
  }

  const BorderSize RTextEditor::getBorder() const throw()
  {
    return borderSize;
  }
  ...
  {
    return indexAtPosition( (x + viewport->getViewPositionX() - leftIndent), (y + viewport->getViewPositionY() - topIndent) );
  }
  ...
  void RTextEditor::paintOverChildren(Graphics& g)
  {
    ...
    if(textToShowWhenEmpty.isNotEmpty() && (!hasKeyboardFocus(false)) && getTotalNumChars() == 0)
    {
      ...
      //g.setFont(getFont());
      if( isMultiLine() )
        drawBitmapFontText(g, 0, 0, textToShowWhenEmpty, &boldFont10px, getTextColour());
      else
        drawBitmapFontText(g, leftIndent, topIndent, textToShowWhenEmpty, &boldFont10px, getTextColour());
    }
  ...
  bool RTextEditor::keyPressed(const KeyPress& key)
  {
    ...
    {
      newTransaction();
      moveCursorTo(indexAtPosition(cursorX, cursorY + cursorHeight + viewport->getViewHeight()), key.getModifiers().isShiftDown());
    }
    else if(key.isKeyCode(KeyPress::pageUpKey) && isMultiLine())
    {
      newTransaction();
      moveCursorTo(indexAtPosition(cursorX, cursorY - viewport->getViewHeight()), key.getModifiers().isShiftDown());
    }
    ...
    else if( key.isKeyCode(KeyPress::endKey) )
    {
      newTransaction();
      if( isMultiLine() && !moveInWholeWordSteps )
        moveCursorTo(indexAtPosition(textHolder->getWidth(), cursorY), key.getModifiers().isShiftDown());
      else
    ...
    for(int i=listeners.size(); --i >= 0;)
    {
      ...
      if(tl != 0)
      {
        ...
        case textChangeMessageId:
          tl->rTextEditorTextChanged(*this);
          break;
        case returnKeyMessageId:
          tl->rTextEditorReturnKeyPressed(*this);
          break;
        case escapeKeyMessageId:
          tl->rTextEditorEscapeKeyPressed(*this);
          break;
        case focusLossMessageId:
          tl->rTextEditorFocusLost(*this);
          break;
        default:
          jassertfalse
            break;
  ...
  {
    setMouseCursor(MouseCursor (isReadOnly() ? MouseCursor::NormalCursor : MouseCursor::IBeamCursor));
    repaint();
  }
  ...
  void RTextEditor::insert(const String& text, const int insertIndex, BitmapFont const* font, const Colour& colour, UndoManager* const um, 
    const int caretPositionToMoveTo) throw()
  {
    if(text.isNotEmpty())
    {
      if(um != 0)
        um->perform(new RTextEditorInsertAction(*this, text, insertIndex, font, colour, caretPosition, caretPositionToMoveTo));
      else
      {
  ...
  void RTextEditor::remove(const int startIndex, int endIndex, UndoManager* const um, const int caretPositionToMoveTo) throw()
  {
    if(endIndex > startIndex)
    {
      ...
      if(um != 0)
      {
        ...
        um->perform(new RTextEditorRemoveAction(*this, startIndex, endIndex, caretPosition, caretPositionToMoveTo, removedSections));
      }
      else
      {
        for(int i=0; i<sections.size(); ++i)
  ...
  const String RTextEditor::getTextSubstring(const int startCharacter, const int endCharacter) const throw()
  {
    String t;
    int index = 0;
    for(int i=0; i<sections.size(); ++i)
  ...
  {
    return getTextSubstring(getHighlightedRegionStart(), getHighlightedRegionStart() + getHighlightedRegionLength());
  }
  ...
  void RTextEditor::splitSection (const int sectionIndex, const int charToSplitAt) throw()
  {
    ...
    sections.insert(sectionIndex + 1, ((UniformTextSection*) sections.getUnchecked(sectionIndex))->split(charToSplitAt));
  }
