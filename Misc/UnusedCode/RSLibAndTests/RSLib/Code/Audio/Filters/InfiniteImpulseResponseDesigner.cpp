...
void rsInfiniteImpulseResponseDesigner::setMode(int newMode)
{
  if( newMode >= BYPASS && newMode <= PEAK )
    ...
    rsError("newMode must be one of the numbers defined in enum modes");
}
...
void rsInfiniteImpulseResponseDesigner::setPrototypeOrder(int newOrder)
{
  if( newOrder >= 1 )
    ...
    rsError("newOrder must be at least 1");
}
...
void rsInfiniteImpulseResponseDesigner::setLowerFrequency(double newLowerFrequency)
{
  if( newLowerFrequency > 0.0 )
    ...
    rsError("negative frequencies are not allowed");
}
...
void rsInfiniteImpulseResponseDesigner::getPolesAndZeros(rsComplexDbl* poles, rsComplexDbl* zeros)
{
  ...
  if( mode == LOWPASS || mode == HIGHPASS || mode == BANDPASS || mode == BANDREJECT )
    prototypeDesigner.setReferenceGain(-rsInfDouble); 
  else
    prototypeDesigner.setReferenceGain(0.0);

  // allocate temporary memory:
  rsComplexDbl* protoPoles = new rsComplexDbl[prototypeOrder];
  rsComplexDbl* protoZeros = new rsComplexDbl[prototypeOrder];
  ...
  // because the rsPrototypeDesigner returns only one representant for each pair of complex 
  ...
  switch( mode )
  {
  case LOWPASS:    rsPoleZeroMapper::sPlanePrototypeToLowpass(   protoPoles, protoZeros, poles, 
                     zeros, prototypeOrder, wa1);      break;
  case HIGHPASS:   rsPoleZeroMapper::sPlanePrototypeToHighpass(  protoPoles, protoZeros, poles, 
                     zeros, prototypeOrder, wa1);      break;
  case BANDPASS:   rsPoleZeroMapper::sPlanePrototypeToBandpass(  protoPoles, protoZeros, poles, 
                     zeros, prototypeOrder, wa1, wa2); break;
  case BANDREJECT: rsPoleZeroMapper::sPlanePrototypeToBandreject(protoPoles, protoZeros, poles, 
                     zeros, prototypeOrder, wa1, wa2); break;
  case LOW_SHELV:  rsPoleZeroMapper::sPlanePrototypeToLowpass(   protoPoles, protoZeros, poles, 
                     zeros, prototypeOrder, wa1);      break;
  case HIGH_SHELV: 
    {
      // this is ugly - rewrite the prototype design code such that all approximation methods can 
      // be treated uniformly:
      if( prototypeDesigner.needsSpecialHighShelvTransform() )
        rsPoleZeroMapper::sPlanePrototypeToHighShelv( protoPoles, protoZeros, poles, zeros, 
        prototypeOrder, wa1);     
      else
        rsPoleZeroMapper::sPlanePrototypeToHighpass(  protoPoles, protoZeros, poles, zeros, 
        prototypeOrder, wa1); 
    } 
    break;
  case PEAK: rsPoleZeroMapper::sPlanePrototypeToBandpass(protoPoles, protoZeros, poles, zeros, 
               prototypeOrder, wa1, wa2); break;
  default:   rsPoleZeroMapper::sPlanePrototypeToLowpass( protoPoles, protoZeros, poles, zeros, 
               prototypeOrder, wa1);      break;
  };
  ...
  double *b2, double *a1, double *a2)
{
  ...
  // allocate temporary memory:
  rsComplexDbl* poles = new rsComplexDbl[finalOrder];
  rsComplexDbl* zeros = new rsComplexDbl[finalOrder];

  getPolesAndZeros(poles, zeros);
  rsFilterCoefficientConverter::polesAndZerosToBiquadCascade(poles, zeros, finalOrder, b0, b1, b2, 
    a1, a2);
  ...
  double *a1, double *a2, double wc, int numBiquads)
{
  ...
  if( prototypeDesigner.getPrototypeMode() == rsPrototypeDesigner::LOWSHELV_PROTOTYPE )
  {
      ...
      || prototypeDesigner.getApproximationMethod() == rsPrototypeDesigner::ELLIPTIC )
    {
      if( rsIsEven(prototypeDesigner.getOrder()) )
      {
        ...
        normalizeFactor = rsDB2amp(-excessDb/numBiquads);
      }
    }
  }
  else // prototype is lowpass
  {
      ...
      || prototypeDesigner.getApproximationMethod() == rsPrototypeDesigner::ELLIPTIC )
    {
      if( rsIsEven(prototypeDesigner.getOrder()) )
      {
        normalizeFactor = 1.0 / rsDB2amp(prototypeDesigner.getPassbandRipple());
