  ...
  double *directFormCoeffs)
{
  ...
  // in the first element of the reflection coefficient vector is the overall gain-factor - we need
  // to scale the FIR vector with this factor and also use it as leading coefficient, we also need 
  // to invert the sign:
  for(i=1; i<=N; i++)
    a[i] = -a[i] * k[0];
  a[0] = k[0];
...
void rsFilterCoefficientConverter::polesAndZerosToBiquadCascade(rsComplexDbl *poles, int numPoles, 
  rsComplexDbl *zeros, int numZeros, double *b0, double *b1, double *b2, double *a1, double *a2,
...
void rsFilterCoefficientConverter::polesAndZerosToBiquadCascade(rsComplexDbl *poles, 
  rsComplexDbl *zeros, int order, double *b0, double *b1, double *b2, double *a1, double *a2)
{
  int numBiquads;
  if( rsIsEven(order) )
    numBiquads = order/2;
  ...
  double *b1, double *b2, double *a1, double *a2, double *b, double *a)
{
  ...
  for(i=0; i<numBiquads; i++)
  {
    ...
    rsCopyBuffer(aAccu, tmp, N);
    rsConvolve(tmp, N-2, aQuad, 3, aAccu);
    rsCopyBuffer(bAccu, tmp, N);
    rsConvolve(tmp, N-2, bQuad, 3, bAccu);
