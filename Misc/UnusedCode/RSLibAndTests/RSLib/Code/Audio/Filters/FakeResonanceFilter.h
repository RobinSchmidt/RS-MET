#ifndef RS_FAKERESONANCEFILTER_H
#define RS_FAKERESONANCEFILTER_H
...
namespace RSLib
{
  ...
  */
  ...
  class RSLib_API rsFakeResonanceFilter
  {
  ...
  public:
    ...
    void setResonanceShift(double newShift);

    /** Sets the resonance amplitude as a linear gain factor. */
    void setResonanceAmplitude(double newAmplitude);
    ...
    void setResonanceDelay(double newDelay);
    ...
    RS_INLINE double getLowpassOutput(double in);

    /** Computes the resonance signal (lower path) */
    RS_INLINE double getResonanceOutput(double in);
  ...
  protected:
    ...
    double shift;        // resonance pitch-shift with respect to cutoff
    double gr;           // resonance gain as raw factor
    double pr;           // resonance start phase in radians
    double dr;           // resonance decay in seconds
    double decayByFreq;  // scaling of decay-time by resonance frequency
    double ar;           // resonance attack as factor 0 < k < 1 to be applied to dr to arrive at
                         // the absolute attack time
    double delay;        // resonance delay
  ...
  RS_INLINE double rsFakeResonanceFilter::getSample(double in)  
  {
    double yl = getLowpassOutput(in);
    double yr = getResonanceOutput(in);
  ...
  RS_INLINE double rsFakeResonanceFilter::getLowpassOutput(double in)
  {
    double yl; 
  ...
  RS_INLINE double rsFakeResonanceFilter::getResonanceOutput(double in)
  {
    double yr;
    yr = hpf.getSample(in);
    yr = dl.getSample(yr);
