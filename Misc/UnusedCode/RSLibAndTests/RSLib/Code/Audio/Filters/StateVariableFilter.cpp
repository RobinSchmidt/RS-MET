...
void rsStateVariableFilter::calcCoeffs()
{
  ...
  case LOWSHELF:
    {
      double A = rsSqrt(G);
      g /= rsSqrt(A);               // scale SVF-cutoff frequency for shelvers
  ...
  case MORPH_LP_BP_HP:
    {
      ...
      double s = G * rsSqrt((R2*R2) / (cL*cL + cB*cB + cH*cH - 2*cL*cH));
      cL *= s; cB *= s; cH *= s;
    }
    break;
...
double rsStateVariableFilter::bandwidthToR(double B)
{
  ...
  double r  = gl/g;            // ratio between warped lower bandedge- and center-frequencies
                               ...
                               // center-frequencies
  return rsSqrt((1-r*r)*(1-r*r)/(4*r*r));
}
