#ifndef RS_INTERPOLATOR_H
#define RS_INTERPOLATOR_H
...
namespace RSLib
{
  ...
  */
  ...
  public:
    ...
    RS_INLINE double getSample(double x, double *y);
    ...
    RS_INLINE double getSampleAllpass(double x, double *y);
    ...
    because the Interpolator class sees it from the viewpoint of a phase-accumulating oscillator
    whereas the paper deals with delaylines. */
    RS_INLINE double getSampleWarpedAllpass(double x, double *y);

    /** Returns the nearest neighbour sample - that is y[0] when x < 0.5 and y[1] otherwise. */
    static RS_INLINE double getSampleNearestNeighbour(double x, double *y);
    ...
    between 0...1. */
    static RS_INLINE double getSampleLinear(double x, double *y);

    /** Calclulates one output sample by linearly interpolating between y[0] and y[1] at
    position x. y should point to an array of at least two values, x should be a number between
    0...1. */
    static RS_INLINE double getSampleLinear(double x, float *y);



    static RS_INLINE double getSampleCubicZeroDerivative(double x, double *y);
    ...
    static RS_INLINE double getSampleParabolic4p2o2x(double x, double *y);
    ...
    static RS_INLINE double getSampleOptimal6p5o2x(double x, double *y);
  ...
  RS_INLINE double rsInterpolator::getSample(double x, double *y)
  {
  ...
  RS_INLINE double rsInterpolator::getSampleAllpass(double x, double *y)
  {
    double result  = y[0] + x*y[1] - 0.999*x*previousOutput;
    previousOutput = result;
    return result;
    // multiplication of feedback with 0.999 avoids self-oscillation at fs/2
  }

  RS_INLINE double rsInterpolator::getSampleWarpedAllpass(double x, double *y)
  {
  ...
  RS_INLINE double rsInterpolator::getSampleNearestNeighbour(double x, double *y)
  {
    if( x < 0.5 )
      return y[0];
    else
  ...
  RS_INLINE double rsInterpolator::getSampleLinear(double x, double *y)
  {
    return y[0] + x*(y[1]-y[0]);
  }

  RS_INLINE double rsInterpolator::getSampleLinear(double x, float *y)
  {
    return y[0] + x*(y[1]-y[0]);
  }

  RS_INLINE double rsInterpolator::getSampleCubicZeroDerivative(double x, double *y)
  {
    double d = y[1]-y[0];
    return d*x*x*(3.0-2.0*x) + y[0];
  }

  RS_INLINE double rsInterpolator::getSampleParabolic4p2o2x(double x, double *y)
  {
  ...
  RS_INLINE double rsInterpolator::getSampleHermite4p3o(double x, double *y)
  {
  ...
  RS_INLINE double rsInterpolator::getSample2ndOsculating4p5o(double x, double *y)
  {
  ...
  RS_INLINE double rsInterpolator::getSampleOptimal4p3o2x(double x, double *y)
  {
  ...
  RS_INLINE double rsInterpolator::getSampleOptimal6p5o2x(double x, double *y)
  {
    doubleA z,
